...
def main():
    ...
    if not os.path.isfile("config/config.toml"):
        ...
        log.fatal("A config file has been created in config/config.toml."
                  " Customize it, then restart mercy!")
        exit(1)
    ...
    with open("config/template_config.toml", encoding="utf8") as template_cfg_file, \
        ...
        if not template_cfg.cmplog(user_cfg):
            log.fatal("There were errors while parsing the config.toml file. Please fix them and restart mercy!")
            exit(2)
        else:
    ...
    me = bot.get_me()
    if me is None:
        logging.fatal("The token you have entered in the config file is invalid. Fix it, then restart mercy.")
        sys.exit(1)
...
#     This replicates your original update-handling logic.
#     """
#     if update.message is not None:
#         if update.message.chat.type != "private":
#             log.debug(f"Received a message from a non-private chat: {update.message.chat.id}")
...
#     """
#     Asynchronous handler for webhook POST requests from Telegram.
#     """
#     try:
...
#     # Process the update (synchronously) using your existing logic.
...
#              open("config/config.toml", "w", encoding="utf8") as user_cfg_file:
#             user_cfg_file.write(template_cfg_file.read())
#         log.fatal("A config file has been created in config/config.toml. Customize it, then restart mercy!")
#         exit(1)
...
#     me = bot.get_me()
#     if me is None:
#         log.fatal("The token you have entered in the config file is invalid. Fix it, then restart mercy.")
#         sys.exit(1)
#     log.debug("Bot token is valid!")
...
#     log.info(f"@{me.username} is starting with webhook!")

#     # Set the webhook (make sure the URL in your config is HTTPS and publicly accessible)
#     webhook_url = user_cfg["Telegram"]["webhook_url"]  # e.g., "https://yourserver.com/webhook"
#     bot.set_webhook(url=webhook_url)

#     # Start the aiohttp web server to handle incoming webhook requests
#     app = asyncio.run(init_app())
#     web.run_app(app, host="0.0.0.0", port=user_cfg["Telegram"].get("port", 5000))
