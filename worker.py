...
class Worker(threading.Thread):
    ...
    def __order_menu(self):
        ...
        You are an assistance in helping to shape various institutions in nigeria.
        DBI means Digital bridge institute
        MFM means mountain of fire and miracles ministies
        RCCG mean Redeemed Christian Church of God etc.
        If users enter a prompt in code e.g mfm, utako, generate the full meaning(mountain of fire and miracles ministries, utako)
        Format the user's query into the following structure:

            From: [Starting Location]
            To: [Destination]

            - Query: "Going to access bank from zenith bank"
            - Output:
            From: [Extracted Starting Location]
    ...
    #     keyboard = [[telegram.KeyboardButton(self.loc.get("menu_get_app"))],
    ...
    #     # Wait for a reply from the user
    #     selection = self.__wait_for_specific_message([
    #         self.loc.get("menu_get_app"),
    #     ], cancellable=True)

    #     # If the user selects "Get App", open the unique URL
    #     if selection == self.loc.get("menu_get_app"):
    #         user_url = f"https://t.me/niceshopping_bot/w30z?start={self.chat.id}"
    #         log.debug(f"Opening: {user_url}")
    #         webbrowser.open(user_url)
    #         self.bot.send_message(self.chat.id, self.loc.get("get_app"))
        
    #     # If the user selects "Contact Shopkeeper"
    ...
    #     # If the user selects "Get App", send them an inline button with a URL
    #     if selection == self.loc.get("menu_get_app"):
    #         user_url = f"https://paystack.com/pay/judx07x776"
    #         log.debug(f"Sending URL button: {user_url}")

    #         # Create an inline button that opens the link when clicked
    #         keyboard = [[InlineKeyboardButton("Open App 🚀", url=user_url)]]
    #         reply_markup = InlineKeyboardMarkup(keyboard)

    #         # Send the button to the user
    #         self.bot.send_message(self.chat.id, "Add funds:", reply_markup=reply_markup)
